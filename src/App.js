function Login() {
  return (
    <div className="flex items-center place-content-center flex-wrap bg-slate-700 w-[25rem] aspect-[1/1.05] rounded-[1rem] p-4 [&>input]:aspect-[1/0.2] [&>input]:w-[20rem]">
      ...
      <input className="border-none rounded-[1rem] hover:rounded-[0.25rem] p-[0.65rem] bg-blue-600 hover:bg-blue-500 m-[0.5rem] text-white focus:outline-none text-center text-3xl font-semibold cursor-pointer" type="button" value="Create Account"></input>
    </div>
  );
}
...
function App() {
  return (
    <div className="w-screen h-screen p-0 m-0 bg-slate-900 flex items-center place-content-center focus:[&_*]:outline-none [&_*]:transition-all">
      <Login />
    </div>
  );
}
